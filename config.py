"""
VEILGUARD — central configuration.
...
# You can also set it via env var VEILGUARD_REGION.
AWS_REGION = os.environ.get("VEILGUARD_REGION", "ap-south-1")

# ---- Project prefix --------------------------------------------------------
# Every decoy VEILGUARD creates gets this prefix so you can always tell
# your traps apart from real resources (and clean them up easily).
DECOY_PREFIX = "veilguard-decoy"
...
# Set via env var VEILGUARD_ALERT_EMAIL before running setup_billing_alarm.py
# (keeps your real address out of the committed file).
ALERT_EMAIL = os.environ.get("VEILGUARD_ALERT_EMAIL", "YOUR_EMAIL@example.com")
...
# Path to the Terraform executable. VEILGUARD calls Terraform through
# Python, so this works even if `terraform` isn't on your PATH.
# On Windows this is typically the full path to terraform.exe.
TERRAFORM_BIN = os.environ.get("VEILGUARD_TERRAFORM_BIN", r"C:\terraform\terraform.exe")
...
BREACH_SNS_TOPIC = "veilguard-breach-alerts"

# ---- Intelligence (Part 6) -------------------------------------------------
# The AI incident report is OPTIONAL. If an Anthropic API key is present
# (env var ANTHROPIC_API_KEY), VEILGUARD writes an AI summary. Otherwise it
# falls back to a clean template report — so the project never depends on a key.
ANTHROPIC_MODEL = os.environ.get("VEILGUARD_AI_MODEL", "claude-sonnet-4-6")

# ---- LLM provider (RAG chat + agent) ---------------------------------------
# VEILGUARD's LLM features work with either Anthropic or an OpenAI-compatible
...
LLM_PROVIDER = os.environ.get("VEILGUARD_LLM_PROVIDER", "openrouter")

# OpenRouter (OpenAI-compatible) settings.
OPENROUTER_BASE_URL = "https://openrouter.ai/api/v1"
# The model id. Nemotron 3 Super is free and supports a large context.
LLM_MODEL = os.environ.get(
    "VEILGUARD_LLM_MODEL", "nvidia/nemotron-3-super-120b-a12b:free")
